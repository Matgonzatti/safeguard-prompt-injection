...
- 👥 **Two User Roles**:
  - `matheusgonzatti` (admin) - Can access file system tools
  - `lenise` (member) - Cannot access file system tools
...
```bash
npm run chat -- --user lenise --unsafe
```
...
```bash
npm run chat -- --user matheusgonzatti
```
